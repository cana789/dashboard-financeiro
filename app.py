...
with st.sidebar:
    ...
    st.caption("Envie os 3 arquivos de uma vez. O app identifica cada um automaticamente.")
        ...
        for arq in arquivos:
            ...
            try:
                if tipo == "empenhos":
                    ...
                    st.success(f"✓ Empenhos: {len(st.session_state.empenhos)} registros")
                elif tipo == "restos":
                    st.session_state.restos = parse_restos(raw)
                    st.success(f"✓ Restos a pagar: {len(st.session_state.restos)} registros")
                elif tipo == "saldo":
                ...
                st.error(f"Erro ao processar {arq.name}: {exc}")

        if erros:
            st.warning(f"Arquivo(s) não reconhecido(s): {', '.join(erros)}")
