...
## 5-B. AUDITORIA — OBSERVAÇÕES E HISTÓRICO (JUN/2026)

- **Toda página de módulo** termina com as abas 📝 OBSERVAÇÕES e 🗂️ HISTÓRICO DO MÓDULO (`secao_observacoes_historico(modulo)` — uma única chamada no fim do `app.py` cobre os 5 módulos, pois `modulo` guarda a seleção atual)
- **Observações:** registradas com usuário (NIP) + data/hora; **exclusão é LÓGICA** (`Ativa=False` + `ExcluídaPor`/`ExcluídaEm`) — some do painel mas permanece no expander “Observações excluídas” e no log
- **Log (`registrar_log`)**: registra TUDO — login, uploads (Financeiro/Patrimônio/CSVs), inclusão manual de empenho, edições na tabela de processos (com diff: incluídos/excluídos/alterados por ID), todas as ações de Tarefas (inclusão, alteração, conclusão, avanço kanban, exclusão, edição em tabela, import), observações, limpeza de dados e geração de relatório
- **Anti-spam de logs:** `_log_upload_unico` usa assinatura (nome+tamanho) em `st.session_state._upload_sigs` — o file_uploader do Streamlit repete os arquivos a cada rerun e sem o guard cada rerun geraria novo log; no Patrimônio o guard também **evita reprocessar os PDFs pesados**
- **Chaves `log` e `observacoes`** entram em `CHAVES_PERSISTENCIA` mas são **preservadas** pelo botão “Limpar dados salvos” (`CHAVES_AUDITORIA`) — auditoria é registro permanente
- Histórico exportável em CSV por módulo; eventos do módulo “Geral” aparecem em todos

## 5-C. FINANCEIRO — PROVISÃO FORA DOS GRÁFICOS DE EXECUÇÃO (JUN/2026)

- Provisão/destaque **continua nos cards** (Provisão Recebida, Crédito Disponível) e nos denominadores das taxas/gauges
- Removida dos gráficos de execução: funil (`grafico_funil(emp, liq, pago)` — % agora relativos ao empenhado), evolução mensal (`grafico_evolucao` só Empenhado×Pago) e `grafico_pi` (ordenado por Empenhado)
- **Gráfico exclusivo** `grafico_provisao(df_mensal)`: linha/área da Provisão recebida + linha pontilhada do Crédito disponível, seção própria “Provisão / destaque de crédito” na Visão Geral
...
- [x] Relatórios PDF exportáveis — Relatório Executivo de governança implementado JUN/2026

### Prioridade baixa / ideias
- [ ] Perfis de acesso (admin × consulta; visibilidade por divisão)
- [x] Log de auditoria (quem alterou o quê) — implementado JUN/2026
- [ ] Refinamento mobile
- [ ] Tema claro opcional
...
**Implementado na 2ª rodada (JUN/2026):**
- Lei 9.784/99: coluna “Prazo da Fase” (dd/mm/aaaa) na tabela de processos da Obtenção (`normalizar_processos` migra dados antigos), com alertas automáticos de prazo vencido/a vencer em 7 dias e expander com tabela de prazos de referência (`PRAZOS_9784`: arts. 24, 26 §2º, 42, 49, 54, 59)
- Circular Governança 9/2025: **Relatório Executivo em PDF** no Painel Geral (`gerar_relatorio_pdf`, reportlab 4.2.5, azul institucional #1F3864), estruturado em 1. AVALIAR (KPIs), 2. DIRECIONAR (atenções + observações ativas), 3. MONITORAR (evolução mensal + execução por submeta GOLF)
