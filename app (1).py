...
def parse_empenhos(file_bytes):
    ...
    # Normaliza PI: remove espaços e aspas residuais do TSV
    df["PI"] = df["PI"].astype(str).str.strip().str.strip('"')
    return df
...
def parse_restos(file_bytes):
    ...
    df = pd.read_csv(
        StringIO("\r\n".join(data_lines)),
        sep="\t",
        names=["NE", "CNPJ", "Favorecido", "Ano", "Nat_Desp", "Desc_Nat",
               "PI", "Desc_PI", "RP_Inscrito", "RP_A_Liquidar", "RP_Liquidado",
               "RP_Liq_A_Pagar", "RP_Pago", "RP_A_Pagar"],
        on_bad_lines="skip",
    )
    df = df[df["NE"].str.contains("NE", na=False)].copy()
    for col in ["RP_Inscrito", "RP_A_Liquidar", "RP_Liquidado",
                "RP_Liq_A_Pagar", "RP_Pago", "RP_A_Pagar"]:
        df[col] = df[col].apply(parse_br)
    df["NE_display"] = df["NE"].str.extract(r"(NE\d+)").fillna(df["NE"])
    df["Favorecido"] = df["Favorecido"].str.strip().str.title()
    ...
    for block in blocks[1:]:
        ...
        try:
            df_tmp = pd.read_csv(
                StringIO("\r\n".join(data_lines)),
                sep="\t",
                names=["PI", "Desc_PI", "Nat_Desp", "Desc_Nat",
                       "Provisao", "Credito_Disp", "Empenhado",
                       "A_Liquidar", "Liquidado", "Liq_A_Pagar", "Pago"],
                on_bad_lines="skip",
            )
            for col in ["Provisao", "Credito_Disp", "Empenhado",
                        "A_Liquidar", "Liquidado", "Liq_A_Pagar", "Pago"]:
                df_tmp[col] = df_tmp[col].apply(parse_br)
            # Normaliza PI: remove espaços e aspas residuais do TSV
            df_tmp["PI"] = df_tmp["PI"].astype(str).str.strip().str.strip('"')
...
with tabs[3]:
    if df_emp.empty and df_sg.empty:
        ...
        if not df_sg.empty:
            ...
            df_pi = df_sg[df_sg["Mes"] == ultimo_mes].groupby(["PI", "Desc_PI"]).agg(
            ...
            df_pi = df_emp.groupby(["PI", "Desc_PI"]).agg(
                Empenhado=("Empenhado", "sum"),
        ...
        # Normaliza PI nos dois lados para evitar falha de tipo/espaço no merge
        df_pi["PI"] = df_pi["PI"].astype(str).str.strip().str.strip('"')

        # Merge com liquidado dos empenhos
        if not df_emp.empty:
            df_emp_norm = df_emp.copy()
            df_emp_norm["PI"] = df_emp_norm["PI"].astype(str).str.strip().str.strip('"')
            liq_pi = df_emp_norm.groupby("PI").agg(Liquidado_emp=("Liquidado", "sum")).reset_index()

            # Remove coluna Liquidado do df_pi caso já exista (vinda do saldo geral)
            # para evitar colunas duplicadas (_x/_y) no merge
            if "Liquidado" in df_pi.columns:
                df_pi = df_pi.drop(columns=["Liquidado"])

            df_pi = df_pi.merge(liq_pi, on="PI", how="left")
            df_pi = df_pi.rename(columns={"Liquidado_emp": "Liquidado"})
